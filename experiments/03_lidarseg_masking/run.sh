...
# Phase 2 (Experiment 03): LiDAR segmentation masking 実験
# 使い方: bash experiments/03_lidarseg_masking/run.sh [scene_index] [stage]
...
echo "=== Experiment 03: LiDAR Segmentation Masking ==="
...
if [ ! -d "$DATA_DIR" ]; then
    echo "=== Stage 0: LiDAR マスク付きデータのエクスポート ==="
...
if [ "$STAGE" -eq 1 ]; then
    ...
    uv run ns-train splatfacto \
      ...
      --output-dir outputs/lidarseg_stage1_quick_test
    ...
    uv run ns-train splatfacto \
      ...
      --viewer.quit-on-train-completion True \
      --output-dir outputs/lidarseg_stage2_light
    ...
    uv run ns-train splatfacto \
      ...
      --viewer.quit-on-train-completion True \
      --output-dir outputs/lidarseg_stage3_full
...
CONFIG_PATH=$(ls -t outputs/lidarseg_stage${STAGE}_*/${SCENE_NAME}_front_lidar_masked/splatfacto/*/config.yml 2>/dev/null | head -1)
